...
namespace SM_ParaMotor
{
    ...
    public class SM_ParaMotor : MonoBehaviour
    {
        private const float WindowWidth = 200;
        private const float DraggableHeight = 40;
        private const float LeftIndent = 12;
        private const float ContentTop = 20;
        public static SM_ParaMotor Fetch;
        ...
        private void Start()
        {
            _windowRect = new Rect(WindowWidth - 100, Screen.height - 200, WindowWidth, _windowHeight);
        ...
        private void OnGUI()
        {
            ...
            {
                _windowRect = GUI.Window(962962, _windowRect, GuiWindowPara, "");
            }
        }
        ...
        private void CheckKerbal()
        {
            foreach (Part p in FlightGlobals.ActiveVessel.Parts)
            {
                ...
                var paraMotor = p.vessel.FindPartModuleImplementing<ModuleParaMotor>();
                if (paraMotor == null)
                {
                    if (GuiEnabled)
                    {
        ...
        private void GuiWindowPara(int Para)
        {
...
//            line++;
        ...
        {
            string textureDir = "SM_ParaMotor/Plugin/";

            if (!HasAddedButton)
            {
                Texture buttonTexture = GameDatabase.Instance.GetTexture(textureDir + "SM_ParaMotor_icon", false); //texture to use for the button
        ...
        {
            GuiEnabled = true;
            Debug.Log("[SM_ParaMotor]: Showing GPS GUI");
        }

        private void DisableGui()
        {
            GuiEnabled = false;
            Debug.Log("[SM_ParaMotor]: Hiding GPS GUI");
        }
        ...
        private void DrawTitle()
        {
            ...
            GUI.Label(new Rect(0, 0, WindowWidth, 20), "SM Para Motor", titleStyle);
        }
